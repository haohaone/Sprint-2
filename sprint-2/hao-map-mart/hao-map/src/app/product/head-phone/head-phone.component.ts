...
@Component({
  selector: 'app-head-phone',
  templateUrl: './head-phone.component.html',
  styleUrls: ['./head-phone.component.css']
})
export class HeadPhoneComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.title.setTitle('Tai nghe');
    this.getAll()
    this.role = sessionStorage.getItem('roles');
  }

  getAll(){
    this.productService.getAllHeadPhoneList(this.limit, this.name, this.price).subscribe(
      (value: any) => {
        if (value != null) {
